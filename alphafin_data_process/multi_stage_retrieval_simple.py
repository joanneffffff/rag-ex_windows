...
# 需要安装的依赖：pip install faiss-cpu sentence-transformers scikit-learn
try:
    import faiss
    ...
    print(f"请安装必要的依赖: pip install faiss-cpu sentence-transformers scikit-learn")
    print(f"错误: {e}")
    exit(1)
...
class MultiStageRetrievalSystem:
    ...
    3. Reranker: 基于original_context使用TF-IDF进行重排序
    """
    ...
    def _build_faiss_index(self):
        ...
        for idx, record in enumerate(self.data):
            # 组合generated_question和summary作为嵌入文本
    ...
    def _init_tfidf_reranker(self):
        """初始化TF-IDF reranker"""
        print("正在初始化TF-IDF reranker...")
        
        # 收集所有original_context用于TF-IDF
        contexts = []
        for record in self.data:
            if record.get('original_context'):
                contexts.append(record['original_context'])
        ...
        print(f"TF-IDF reranker初始化完成，特征数: {self.tfidf_matrix.shape[1]}")
    ...
    def rerank(self, 
        ...
        for i, (idx, faiss_score) in enumerate(candidate_results):
            if i < len(candidate_indices) and candidate_indices[i] == idx:
                # 找到对应的TF-IDF分数
                tfidf_score = 0.0
                for j, context_idx in enumerate(range(len(self.data))):
                    if context_idx == idx and j < len(similarities):
                        tfidf_score = similarities[j]
                        break
                
                # 简单的分数组合：加权平均
                combined_score = 0.7 * faiss_score + 0.3 * tfidf_score
