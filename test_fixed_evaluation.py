...
def test_fixed_evaluation():
    ...
    try:
        ...
        print(f"   原始知识库大小: {len(chinese_chunks)} 中文, {len(english_chunks)} 英文")
        ...
        for sample in alphafin_eval[:5]:  # 只添加前5个样本
        ...
        for sample in tatqa_eval[:5]:  # 只添加前5个样本
            if sample.get('context'):
                eval_doc = DocumentWithMetadata(
                    content=json.dumps({
                        'query': sample.get('query', ''),
        ...
        print(f"   添加了 {len(alphafin_eval[:5])} 个中文评估样本")
        print(f"   添加了 {len(tatqa_eval[:5])} 个英文评估样本")
        print(f"   最终知识库大小: {len(chinese_chunks)} 中文, {len(english_chunks)} 英文")
